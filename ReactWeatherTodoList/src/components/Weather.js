...
const Weather = () => {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        ...
        const apiKey = "c00ad92e2fd72c4fea1afc8a1ceab792";
        if (!apiKey) {
  ...
  const getCurrentPosition = () => {
    ...
    return new Promise((resolve, reject) => {
      if(navigator.geolocation){
        ...
        reject("There are some errors with your API key");
      }
    })
  };
  ...
  const getWeatherIconUrl = (iconCode) => {
    const iconType = iconCode.endsWith("n")
              ...
              dataSource={[
                {
                  ...
                  desc: `${kelvinToCelsius(
                    weatherData.main.temp_max
                  )} °C / ${kelvinToCelsius(weatherData.main.temp_min)} °C`,
                },
                {
